#!/usr/bin/env node
/**
 * hidra — enruta cada tarea al agente/modelo/máquina más barato CAPAZ y DISPONIBLE.
 *
 ...
 *   4) EXECUTE -> local (or a remote worker over ssh). 5) LOG (structured NDJSON).
 *
 *   mesh "<tarea>"            mesh --dry "<tarea>"        mesh --route-only "<tarea>"
 *   hidra --intent X --complexity hard "<task>"     hidra --privacy sensitive "<task>"
...
if (argv[0] === "use") {
  ...
  console.log(`✓ Backend de pago del mes: ${paid}. (hidra lo priorizará donde aplique).`);
  process.exit(0);
}
if (argv[0] === "status") {
...
if (argv[0] === "doctor") {
  console.log("hidra doctor — diagnóstico (sin gastar):\n");
...
if (argv[0] === "health" || argv[0] === "salud") {
  ...
  const n = L.length;
  if (!n) { console.log("hidra health — el log está vacío todavía (nada que auditar)."); process.exit(0); }
  ...
  console.log(`hidra health — ¿el router decidió BIEN? (lee hidra.log, coste 0)`);
...
// onlyMachine: un candidato gateado solo cuenta si su máquina coincide (e.g. a model pinned to one worker).
...
const OLU = join(HOME, ".cache", "hidra-ollama-lastuse");
const OPID = join(HOME, ".cache", "hidra-ollama-watchdog.pid");
...
if (!task) { console.error('Uso: hidra [--dry|--route-only] [--intent X] [--complexity ..] [--privacy normal|sensitive] "<task>"'); process.exit(1); }
...
// cuenta si coincide con la máquina de ejecución (e.g. a model pinned to one worker).
...
if (meta.privacy === "sensitive") {
  ...
  if (!candidates.length) { console.error("✗ ABORT (exit 3): privacy=sensitive but no LOCAL backend is available. Sensitive data must NEVER go to the cloud. Start a local model and retry."); process.exit(3); }
}
...
// ---------- decisión ----------
console.error("── hidra ──");
console.error(`task      : ${task.slice(0, 80)}${task.length > 80 ? "…" : ""}`);
console.error(`classify  : intent=${meta.intent} complexity=${meta.complexity} privacy=${meta.privacy}`);
console.error(`route     : tier=${tierName} → ${pick.backend}/${pick.model} @ ${machine}` + (fellBack ? `  (fallback: ${preferred.backend} no disponible)` : ""));
console.error(`reason    : ${route.why}`);
...
function runOpenrouter(model, p) {
  ...
  const body = JSON.stringify({ model, messages: [{ role: "user", content: p }], stream: false });
  const r = spawnSync("curl", ["-s", "https://openrouter.ai/api/v1/chat/completions", "-H", `Authorization: Bearer ${key}`, "-H", "Content-Type: application/json", "-H", "X-Title: hidra", "-d", body], { encoding: "utf8", timeout: 180000 });
...
function runFusion(task, pro, dry) {
  ...
  console.error("──────────────");
  let exit = 1;
  const r = spawnSync("curl", ["-s", "--max-time", "290", "https://openrouter.ai/api/v1/chat/completions", "-H", `Authorization: Bearer ${key}`, "-H", "Content-Type: application/json", "-H", "X-Title: hidra-fusion", "-d", JSON.stringify(body)], { encoding: "utf8", timeout: 300000, maxBuffer: 16 * 1024 * 1024 });
...
// SSH endurecido: if the remote worker is off, fail fast (~4s) instead of
...
// Uses ssh with ConnectTimeout for the optional remote worker.
...
for (let i = 0; i < candidates.length; i++) {
  ...
  console.error(`execute   : ${cand.backend}/${cand.model} @ ${machine}${i > 0 ? "  (fallback en cascada)" : ""}`);
  console.error("──────────────");
